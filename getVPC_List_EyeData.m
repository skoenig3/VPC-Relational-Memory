function getVPC_List_EyeData(datafile,itmnum)
%modified from getSCM_EyeDat by Seth Konig 6/9/2015
%function imports VPC eye data and calibrates it. 

samprate = 5;

ITMFile = ['C:\Users\seth.koenig\Documents\MATLAB\VPC Relational Memory\Item and CND Files\2IML' num2str(itmnum) 'W.itm'];
CNDFile = ['C:\Users\seth.koenig\Documents\MATLAB\VPC Relational Memory\Item and CND files\2IML.cnd'];
...
for trial = 1:size(img_cnd,2)
    if ~isnan(img_cnd(1,trial))
        ...
        imgnum1 = str2double(itmline1{1}{end}(7:end-4));
        imgnum(1,trial) = imgnum1;
        
        itmline2 = textscan(itmfil(img_cnd(2,trial)+6,:),'%s');
        imgnum2 = str2double(itmline2{1}{end}(7:end-4));
